<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="排行榜编号" prop="paixingbangbianhao">
					<el-input v-model="ruleForm.paixingbangbianhao" placeholder="排行榜编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.paixingbangbianhao" label="排行榜编号" prop="paixingbangbianhao">
					<el-input v-model="ruleForm.paixingbangbianhao" placeholder="排行榜编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="排名" prop="paiming">
					<el-input v-model="ruleForm.paiming" placeholder="排名" clearable  :readonly="ro.paiming"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="排名" prop="paiming">
...
export default {
	data() {
		...
		return {
			...
			ruleForm: {
				paixingbangbianhao: this.getUUID(),
				xueshengzhanghao: '',
				xueshengxingming: '',
				paiming: '',
				lurushijian: '',
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='paixingbangbianhao'){
							this.ruleForm.paixingbangbianhao = obj[o];
							this.ro.paixingbangbianhao = true;
							continue;
						}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chuqinpaixingbang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chuqinpaixingbangCrossAddOrUpdateFlag = false;
						 this.parent.search();
